      ...
      <div className="rounded-2xl sm:rounded-3xl overflow-hidden backdrop-blur-xl bg-gradient-to-r from-[#6498fe] via-[#5a87f7] to-[#6498fe] shadow-2xl border border-white/20 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex justify-between h-14 sm:h-16 lg:h-18 items-center">
            ...
            {/* ✅ RIGHT SIDE - Desktop: Dashboard + User + Admin OR Login/Register */}
            <div className="hidden md:flex items-center space-x-3 lg:space-x-4">
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <Link
                    to="/dashboard"
                    className={`px-4 lg:px-5 py-2 rounded-xl text-sm lg:text-base font-bold transition-all duration-300 flex items-center gap-2 shadow-lg hover:shadow-xl ${isActive('/dashboard')
                ...
                // ✅ FIXED: Login/Register Buttons for NON-AUTHENTICATED users
                <div className="flex items-center space-x-3">
                <Link to="/login">
  <button className="border-2 border-white text-white hover:bg-white hover:text-blue-600 rounded-xl font-bold transition-all duration-300 shadow-lg hover:shadow-xl px-4 lg:px-5 py-2">
    Login
  </button>
</Link>
<Link to="/register">
  <button className="bg-white text-blue-600 hover:bg-blue-50 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 px-4 lg:px-5 py-2">
    Register
  </button>
</Link>
      ...
      <div
        className={`md:hidden overflow-hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen ? 'max-h-[500px] opacity-100 mt-2' : 'max-h-0 opacity-0'
          }`}
      >
        <div className="mx-3 rounded-2xl overflow-hidden backdrop-blur-xl bg-white shadow-2xl border border-gray-100">
          <div className="px-3 py-4 space-y-2">

            {/* ✅ Dashboard Link - Mobile (Only if authenticated) */}
            {isAuthenticated && (
              <Link
                to="/dashboard"
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-bold transition-all duration-300 ${isActive('/dashboard')
            ...
            {isAuthenticated ? (
              ...
              // ✅ FIXED: Login/Register Buttons - Mobile
              <div className="space-y-2 pt-2 border-t border-gray-100">
                <Link to="/login" onClick={() => setIsMobileMenuOpen(false)}>
                  <button className="w-full border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl font-bold transition-all duration-300 shadow-md py-3 px-4">
                    Login
                  </button>
                </Link>
                <Link to="/register" onClick={() => setIsMobileMenuOpen(false)}>
                  <button className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-black hover:from-blue-700 hover:to-blue-800 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 py-3 px-4">
                    Register
                  </button>
                </Link>
